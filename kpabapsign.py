...
def get_bus_times():
    try:
        query = urllib.parse.urlencode({"token":"abcd123", "stopcode":"60383"})
        url = "http://services.my511.org/Transit2.0/GetNextDeparturesByStopCode.aspx"
        ...
        for route in routes:
            ...
            if route_code == "22" or route_code== "522":
                for depart_time in route.getElementsByTagName('DepartureTime'):
                    depart_times.append(depart_time.childNodes[0].nodeValue)
                if depart_times != []:
                    output.append("Route %s: %s min" % (route_code, ", ".join(depart_times)))
...
def get_quake_data():
    try:
        ...
        mag = round(float(quake['properties']['mag']),1)
